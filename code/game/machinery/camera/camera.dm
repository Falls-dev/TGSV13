	...
	if(istype(attacking_item, /obj/item/paper))
		...
		for(var/mob/potential_viewer in GLOB.player_list)
			...
			if(isAI(potential_viewer))
				...
				if(user.name == "Unknown")
					to_chat(ai, "<span class='name'[user]</span> holds <a href='byond://?_src_=usr;show_paper_note=[REF(last_shown_paper)];'>\a [item_name]</a> up to one of your cameras ...")
				else
			...
			if (potential_viewer.client?.eye == src)
				...
				to_chat(potential_viewer, "<span class='name'[user]</span> holds <a href='byond://?_src_=usr;show_paper_note=[REF(last_shown_paper)];'>\a [item_name]</a> up to your camera...")
		return
